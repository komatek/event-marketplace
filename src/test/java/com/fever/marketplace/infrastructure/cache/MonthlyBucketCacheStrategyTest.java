...
class MonthlyBucketCacheStrategyTest {
    ...
    void shouldHandleExceptionsGracefully() {
        ...
        assertThat(cacheStrategy.getErrors()).isEqualTo(1); // Should increment error counter
    }
    ...
    void shouldTrackCacheMetrics() {
        ...
        // Mock config for both calls
        when(config.getMaxBucketsPerQuery()).thenReturn(24);

        // Test cache hit - return non-null list
        when(bucketOperations.getBucketEvents(LocalDate.of(2024, 12, 1))).thenReturn(List.of());
        cacheStrategy.getEvents(startsAt, endsAt);

        // Test cache miss - return null
        when(bucketOperations.getBucketEvents(LocalDate.of(2024, 12, 1))).thenReturn(null);
        cacheStrategy.getEvents(startsAt, endsAt);

        // Then
        assertThat(cacheStrategy.getFullHits()).isEqualTo(1);
        assertThat(cacheStrategy.getMisses()).isEqualTo(1);
    }
    ...
    void shouldHandlePartialCacheHitWithMultipleMonths() {
        ...
        // Verify partial hit counter is incremented
        assertThat(cacheStrategy.getPartialHits()).isEqualTo(1);
    }
